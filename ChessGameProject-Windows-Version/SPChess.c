...
int findPositionIA(SPChess* src, SP_COLOR currentPlayer, int depth, int alpha, int beta) {
	int row_o = 0, col_o = 0, row_d = 0, col_d = 0;
...
void spChessLoad(SPChess* src){
    ...
    else
    {
			...
			fscanf_s(chessFile, "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n");
			fscanf_s(chessFile, "<game>\n");
			fscanf_s(chessFile, "<current_turn>%d</current_turn>\n",&a);
			src->currentPlayer = a;
			fscanf_s(chessFile, "<game_mode>%c</game_mode >\n", &src->gameMode);
			if(src->gameMode == '1'){
				fscanf_s(chessFile, "<difficulty>%c</difficulty>\n", &src->diff);
				fscanf_s(chessFile, "<user_color>%d</user_color>\n", &a);
				src->currentPlayer = a;
			}else{
				fscanf_s(chessFile, "<difficulty> </difficulty>\n");
				fscanf_s(chessFile, "<user_color> </user_color>\n");
			}
			fscanf_s(chessFile, "<board>\n");

			char s[30] = "";
			for(i = 0; i < DIM_MATRIX; i++){
				//fscanf_s(chessFile, "%s", s, _countof(s));
				fscanf_s(chessFile, "<row_%c>%c%c%c%c%c%c%c%c</row_%c>\n", &row, 1, &src->board[i][0], 1,  &src->board[i][1], 1, &src->board[i][2], 1, &src->board[i][3], 1, &src->board[i][4], 1, &src->board[i][5], 1, &src->board[i][6], 1, &src->board[i][7], 1, &row, 1);
			...
			fscanf_s(chessFile, "</board>\n");
			fscanf_s(chessFile, "<general>\n");
			fscanf_s(chessFile, "<player_undos>%d</player_undos>\n", &playerHistory.n);
			fscanf_s(chessFile, "<player_undos>%d</player_undos>\n", &IAHistory.n);
			for(i = 0; i < playerHistory.n; i++){
				fscanf_s(chessFile, "<move>\n");
				fscanf_s(chessFile, "<player_history>(%d,%d) to (%d,%d)</player_history>\n", &playerHistory.data[i].row_o, &playerHistory.data[i].col_o, &playerHistory.data[i].row_d, &playerHistory.data[i].col_d);
				fscanf_s(chessFile, "<piece_o>%c</piece_o>\n", &playerHistory.data[i].piece_o);
				fscanf_s(chessFile, "<piece_d>%c</piece_d>\n", &playerHistory.data[i].piece_d);
				fscanf_s(chessFile, "<ia_history>(%d,%d) to (%d,%d)</ia_history>\n", &IAHistory.data[i].row_o, &IAHistory.data[i].col_o, &IAHistory.data[i].row_d, &IAHistory.data[i].col_d);
				fscanf_s(chessFile, "<piece_o>%c</piece_o>\n", &IAHistory.data[i].piece_o);
				fscanf_s(chessFile, "<piece_d>%c</piece_d>\n", &IAHistory.data[i].piece_d);
				fscanf_s(chessFile, "</move>\n");
			}
			fscanf_s(chessFile, "<check>%d</check>\n", &a);
			src->check = a;
			fscanf_s(chessFile, "<checkmate>%d</checkmate>\n", &a);
			src->checkMate = a;
			fscanf_s(chessFile, "</general>\n");
			fscanf_s(chessFile, "</game>\n");
